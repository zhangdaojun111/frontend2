...
let config = {
    ...
    actions:{
        ...
        initPageController:function () {
            this.data.dataPageController = new dataPagination;
            let $parent1 = this.el.find('.data-page-control');
            this.data.dataPageController.render($parent1);
            let str = `共 ${this.data.dataCount} 条查询数据`;
            let $span = $("<span class='data-count-display'>").html(str);
            $parent1.append($span);

            this.data.attachmentPageController = new dataPagination;
            let $parent2 = this.el.find('.attachment-page-control');
            this.data.attachmentPageController.render($parent2);
            str = `共 ${this.data.attachmentCount} 条查询数据`;
            let $span2 = $("<span class='attachment-count-display'>").html(str);
            $parent2.append($span2);
        },
        sendSearch:function () {
            let searchData = {
            ...
            GlobalService.sendSearch(searchData).done((result) => {
                console.log("search data result: ",result);
                if(result.success === 1){
                    ...
                    let totalPage = Math.ceil(tempData.total/24);

                    this.data.dataPageController.actions.setPagination(2,1);
                }else{
            ...
            //发起第二次搜索请求，搜索附件
            GlobalService.sendSearch(searchData).done((result) => {
                console.log("search attachment result: ",result);
                if(result.success === 1){
                    // let tempData = result;                           //开全局搜索接口后使用
                    let tempData = this.data.test_attachment_result;    //测试使用
                    this.data.attachmentCount = tempData.total;
                    let str = `共 ${this.data.attachmentCount} 条查询数据`;
                    this.el.find('.attachment-count-display').html(str);
                    this.actions.displayAttachmentResult(tempData);
                    let totalPage = Math.ceil(tempData.total/18);
                    this.data.attachmentPageController.actions.setPagination(totalPage,1);
                }else{
