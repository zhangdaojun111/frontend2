...
let config = {
	...
	actions: {
			...
			if(/\.(png|PNG)$/.test(imgFile.name)){
				if(imgFile){
					...
					FR.onload = function (event){
						...
						_this.actions.sealChange({"base64":imgstr2});
					};
		...
		async sealChange(msg){
			await workflowService.addStmpImg(msg);
			let res =await workflowService.getStmpImg();
			this.actions.changeImg(res);
			this.actions.init();
		},
		/**
		 * 修改盖章图片列表
		...
		async delImg(stmp){
			...
			workflowService.delStmpImg({"file_id":msg});
			let res =await workflowService.getStmpImg();
			this.actions.changeImg(res);
			this.actions.init();
		},
